...
export default function ContestantLibraryPage() {
  ...
  const fetchContestants = async () => {
    try {
      const data = await contestantApi.getAll()
      setContestants(data)
    } catch (error) {
      toast({
        title: 'Error',
        description: error instanceof Error ? error.message : 'Failed to load contestants',
        variant: 'destructive',
      })
    } finally {
      setLoading(false)
  ...
  const filteredContestants = contestants.filter((contestant) =>
    `${contestant.ner} ${contestant.ovog}`.toLowerCase().includes(searchQuery.toLowerCase()) ||
  ...
  const openEditModal = (contestant: Contestant) => {
    ...
    setFormData({
      ...
      tursunUdur: new Date(contestant.tursunUdur).toISOString().split('T')[0],
  ...
  const handleSaveContestant = async () => {
    ...
    try {
      ...
      if (editingContestant) {
        await contestantApi.update(editingContestant._id, submitData)
        setContestants((prev) =>
          prev.map((c) => (c._id === editingContestant._id ? { ...c, ...submitData } : c))
        )
        toast({
          title: 'Success',
        ...
        const newContestant = await contestantApi.create(submitData)
            ...
            {paginatedContestants.length === 0 ? (
              ...
              paginatedContestants.map((contestant) => (
                <TableRow key={contestant._id}>
                  <TableCell className='font-medium'>{contestant.contestantId}</TableCell>
                  <TableCell>{contestant.ovog} {contestant.ner}</TableCell>
                  <TableCell>{contestant.register}</TableCell>
                  <TableCell>{contestant.email}</TableCell>
                  <TableCell>
                    {new Date(contestant.tursunUdur).toLocaleDateString('en-US', {
                      year: 'numeric',
                      month: '2-digit',
                      day: '2-digit',
                    })}
                  </TableCell>
                  <TableCell className='capitalize'>{contestant.gender}</TableCell>
                  <TableCell>
      ...
      <Dialog open={isModalOpen} onOpenChange={setIsModalOpen}>
        <DialogContent className='max-w-2xl max-h-[90vh] overflow-y-auto'>
          ...
          <DialogFooter>
            ...
            <Button onClick={handleSaveContestant} disabled={saving}>
              {saving ? 'Saving...' : editingContestant ? 'Update' : 'Create'}
            </Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
